...
int main(int argc, char** argv) {
    ...
    goal.target_pose.pose.position.x = 1.391;
    goal.target_pose.pose.position.y = -3.447;
    goal.target_pose.pose.orientation.z = -0.053;
    ...
    if (ac.getState() == actionlib::SimpleClientGoalState::SUCCEEDED) {
        ...
        ros::param::set("/reach_put_point", true);
    } else {
